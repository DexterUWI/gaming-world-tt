...
let removeClasses = a => {
	...
	nameValid = () => {
		regName.test(name.value) ? (name.classList.add('is-success'), isFormOK = !0, progress.value += 20) : (name.classList.add('is-danger'), isFormOK = !1, progress.value -= 20)
	},
	userValid = () => {
		regUsername.test(username.value) ? (username.classList.add('is-success'), isFormOK = !0, progress.value += 20) : (username.classList.add('is-danger'), isFormOK = !1, progress.value -= 20)
	},
	emailValid = () => {
		regEmail.test(email.value) ? (email.classList.add('is-success'), isFormOK = !0, progress.value += 20) : (email.classList.add('is-danger'), isFormOK = !1, progress.value -= 20)
	},
	pwdValid = () => {
		regPwd.test(pwd.value) ? (pwd.classList.add('is-success'), isFormOK = !0, progress.value += 20) : (pwd.classList.add('is-danger'), isFormOK = !1, progress.value -= 20)
	},
	confirmValid = () => {
...
$(document).ready(() => {
	$('#registerBtn').click(() => {
		isFormOK ? $('#notify').addClass('is-link') : $('#notify').addClass('is-danger'), $('#notify').addClass('opacityActive'), setTimeout(() => {
			$('#notify').removeClass('opacityActive'), isFormOK ? $('#notify').removeClass('is-link') : $('#notify').removeClass('is-danger')
		}, 3e3)
	}), $('#deleteBtn').click(() => {
		$('#notify').animate({
			opacity: 0
